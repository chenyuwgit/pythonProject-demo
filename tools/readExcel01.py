...
class ExcelData():
    ...
    def __init__(self, excelPath, sheetName="Sheet1"):
        ...
        #self.excel_path = "D:\python-workpase\pythonProject-demo\case\case_excel.xls"  # 指定Excel读取的路径
        #获取路径
        #self.data = xlrd.open_workbook(excelPath)
        self.data = xlrd.open_workbook(excelPath)  # 打开Excel
        print(self.data)
    ...
    def dict_data(self):
        if self.row_num < 1:
            ...
            r = []
            s = {}
            values = self.table.col_values(10)            # 获取第三列作为values值
...
if __name__ == "__main__":
    #sheetName = "购物"
    # sheetName = "case_excel"
    # a = ExcelData(sheetName)
    aa = ExcelData(config.CASE_EXCEL_PATH, sheetName="case_excel")
    print("读取到excel中第二列和第三列对应的数据为：", aa.dict_data())
    #print("读取的数据为列中，把列中中的字典取出来为：", aa.list_to_dict())
    #print("取excel中current_url的具体值为：", a.get_data("method"))  # 后续要取excel中第二列对应的元素属性值，只需要写第二列元素名称即可
